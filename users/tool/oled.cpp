...
OLED::OLED(
    uint32_t RCC_AHB2Periph_GPIOx, GPIO_TypeDef *GPIOx_scl, uint16_t Pin_scl,
    GPIO_TypeDef *GPIOx_sda, uint16_t Pin_sda) {
  RCC_AHB2PeriphClockCmd(RCC_AHB2Periph_GPIOx,ENABLE);

  GPIO_InitTypeDef i2c;
  i2c.GPIO_Mode = GPIO_Mode_OUT;
  i2c.GPIO_PuPd = GPIO_PuPd_NOPULL;
  i2c.GPIO_OType = GPIO_OType_PP;
  i2c.GPIO_Speed = GPIO_Speed_100MHz;
